...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!testModelId) {
      ...
      if (apiKeys.qwen) {
        ...
        return NextResponse.json(
          { 
            error: 'No API Key configured. Please set QWEN_API_KEY environment variable.',
    ...
    const result = await generateTextWithModel(testModelId, testPrompt, { maxTokens: 50 });
    ...
    return NextResponse.json({
      ...
      usage: result.usage,
      configuredProviders: Object.keys(apiKeys).filter(key => apiKeys[key as keyof typeof apiKeys]),
    });
  } catch (error) {
