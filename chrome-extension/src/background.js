      ...
      func: (words) => {
        return new Promise((resolve) => {
          try {
            ...
            title.textContent = '选择一个单词来保存该例句';
...
const analyzeWord = async (word, tabId = null) => {
  try {
    ...
    const enrichmentResponse = await fetch(`${apiBaseUrl}/api/word-enrichment`, {
      method: 'POST',
      headers: {
    ...
    console.log('[analyzeWord] 成功获取单词信息（词性、词根、词族、翻译、例句、音标）');
...
const notifyBoardRefresh = async (sourceTabId) => {
  try {
    ...
    const boardTabs = allTabs.filter((tab) => {
      ...
      return (
        boardUrlPatterns.some((pattern) => url.startsWith(pattern.toLowerCase())) ||
        url.includes('/board')
      ); // 兜底：包含 /board 路径
    });

    console.log(
      '[notifyBoardRefresh] 找到的 board 标签页:',
      boardTabs.length,
      boardTabs.map((t) => t.url)
    );
      ...
      try {
        ...
        if (isAccessibleUrl(tab.url)) {
          try {
            ...
            console.log(
              `[notifyBoardRefresh] 成功通过 executeScript 通知标签页 ${tab.id}（备用方案）`
            );
            successCount++;
          } catch (err) {
...
const handleSaveSelection = async (tabId, url, title, selectedTextRaw) => {
  ...
  if (matchedWord) {
    ...
    // 检查例句是否已存在
    if (
      sentences.some((s) => normalize(s).toLowerCase() === normalize(newSentence).toLowerCase())
    ) {
